...
async function operator(proxies = [], targetPlatform, context) {
  ...
  Object.keys(nameMap).forEach(key => {
    nameMap[key].forEach((proxy, idx) => {
      ...
      // 获取ISP信息
      const isp = proxy._entrance.isp || proxy._entrance.org || proxy._entrance.as || ''
      // 构建新的节点名称，格式：国家 城市 序号 [ISP]
      const ispSuffix = isp ? ` [${isp}]` : ''
      proxy.name = `${key} ${num}${ispSuffix}`.trim()
    })
  })
